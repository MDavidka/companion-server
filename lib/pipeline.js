...
function spawnApp(appName, targetDir, port, framework) {
  ...
  } else if (fw.type === 'static') {
    // Static site: no package.json at all. Serve the directory with `npx serve`
    // (or `dist`/`build` if present from a previous build step). Do NOT run
    // `npm start` — npm walks up the directory tree and would re-launch the
    // runner's own package.json, spawning a second copy of the dashboard.
    let serveDir = targetDir;
    for (const candidate of ['dist', 'build', 'public', 'out']) {
      const p = path.join(targetDir, candidate);
      if (fs.existsSync(p) && fs.statSync(p).isDirectory()) { serveDir = p; break; }
    }
    cmd = 'npx';
    args = ['--yes', 'serve', '-s', serveDir, '-l', `tcp://0.0.0.0:${port}`];
    addLog(appName, `Starting static server: npx serve ${serveDir} on :${port}`);
  } else {
    cmd = 'npm';
    args = ['start'];

...
async function buildApp(appName, targetDir) {
  ...
  // Remove stale node_modules and lockfile in the cloned repo so we resolve fresh from registry
  ...
  addLog(appName, 'Installing dependencies (npm install --registry=https://registry.npmjs.org/)...');
  try {
    await execPromise('npm install --registry=https://registry.npmjs.org/ --no-audit --no-fund', {
      cwd: targetDir,
      maxBuffer: 50 * 1024 * 1024,
    });
  } catch (err) {
    // Fallback: if a specific @radix-ui/react-context-menu version is unresolvable, bump it and retry
    const msg = String(err.stderr || err.message || '');
    if (/ETARGET|No matching version/i.test(msg) && /react-context-menu/i.test(msg)) {
      addLog(appName, 'ETARGET on @radix-ui/react-context-menu — pinning to ^2.0.0 and retrying...');
      try {
        const pkgPath = path.join(targetDir, 'package.json');
        const pkgJson = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
        if (pkgJson.dependencies && pkgJson.dependencies['@radix-ui/react-context-menu']) {
          pkgJson.dependencies['@radix-ui/react-context-menu'] = '^2.0.0';
        }
        if (pkgJson.devDependencies && pkgJson.devDependencies['@radix-ui/react-context-menu']) {
          pkgJson.devDependencies['@radix-ui/react-context-menu'] = '^2.0.0';
        }
